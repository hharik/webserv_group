...
/*   Updated: 2023/06/21 22:57:19 by ajemraou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
response::~response( )
{
	...
	if (header_data->_uploaded == false)
	{
		unlink(header_data->requested_resource.c_str());
	}
	delete status;
...
int	response::CheckTheCGI_Process()
{
	...
	if (status_code == 0)
	{
		/* Returns 0 if CGI mode is disabled or the process does not expire */
	...
	else if (pid == status_code)
	{
		...
		/* If the child is not terminated normally */
		else
		{
			header_data->res_status = 500;
		}
...
void	response::Send_the_Header( int client_fd )
{
	if (header_data->_is_cgi == true)
	{
		if (SUCCESS(header_data->res_status) == true)
		{
		...
		{
			requested_file.open(header_data->requested_resource);
		}
		is_open = true;
	}
	else
...
void	response::SetCGI_Env( std::string script_filename )
{
	...
	Env.push_back("PATH_INFO=" + std::string(agv[1]) + header_data->path_info);
...
void	response::handle_cgi()
{
	...
	/*file to store*/
	start_time = get_time();
	cgi_output = header_data->TempPath + time_date() + header_data->client_index;
...
std::string	response::get_start_line()
{
	...
	else if (header_data->res_status == 505)
		response_content += S505;
...
void	response::Pages()
{
	...
	if (header_data->res_status >= 400)
	{
		...
		if (er_it != server_data->errors.end())
		{
			...
			if (access(er_it->second.c_str(), F_OK) == 0 && access(er_it->second.c_str(), R_OK) == 0)
			{
				return ;
			}
		}
...
void	response::SetCgiStartLine()
{
	...
	/* Does the CGI set a valid status code? */
	status = VALID_STATUS(header_data->res_status);
	if (status == false)
	{
