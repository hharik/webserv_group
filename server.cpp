...
/*   Updated: 2023/06/21 22:48:57 by ajemraou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Server::Create_http_servers()
{
	...
	while( true )
	{
		...
		for(unsigned int i = 0; i < events_nbr; i++)
		{
			...
			{
				if (events[i].filter == EVFILT_READ && user_data->get_is_terminated() == false)
				{
					user_data->get_client()->read_from_socket();
				}
				/* send the response to the client */
				if (events[i].filter == EVFILT_WRITE && user_data->get_is_terminated() == false)
				{
